...
Read the `scan_results.json` file and perform your expert analysis. This is YOUR job as a cybersecurity expert — the tool collects data, you provide the intelligence.

### What to analyze

1. **Every host** — review its open ports, services, versions, and any vulnerability findings
2. **Severity classification** — consider business context:
   - Default password on a firewall = CRITICAL
   - Default password on a printer = HIGH
   - Outdated SSH on an internal-only server = MEDIUM
3. **Positive practices** — identify what the network does RIGHT (strong encryption, good segmentation, updated firmware, etc.)
4. **Risk prioritization** — rank findings by urgency and effort to fix

### Write analysis.json

Save your analysis to the same output directory as the scan results:

```json
{
  "executive_summary": "2-3 paragraph business-language summary. Avoid jargon. Focus on risk posture, key concerns, and overall health. Written for a non-technical executive.",
  "overall_grade": "B+",
  "overall_grade_explanation": "Clear rationale for the grade, referencing specific findings",
  "key_findings": [
    {
      "severity": "critical|high|medium|low|info",
      "title": "Short, clear finding title",
      "description": "Detailed technical description of the issue",
      "affected_device": "IP (Vendor/Hostname)",
      "evidence": "What scan data supports this — port numbers, service versions, Nuclei template matches",
      "business_impact": "What could happen if exploited, in business terms",
  ...
  "positive_practices": [
    {
      ...
      "description": "Specific evidence of good security practice"
    }
  ],
  "priority_actions": [
    {
...
# 3. Read results, write analysis (read_file + write_file)
# Read: <OUTPUT_DIR>/scan_results.json
# Write: <OUTPUT_DIR>/analysis.json

# 4. Render report (bash)
python3 src/tools/network-audit/audit.py --render \
  --scan-data <OUTPUT_DIR>/scan_results.json \
  --analysis <OUTPUT_DIR>/analysis.json \
  --output <OUTPUT_DIR>/report.html

# 5. Email report if user wants (bash)
curl -s -X POST https://localhost:3456/api/gmail/send-report \
  -H "Content-Type: application/json" \
  --cacert /Users/jarvis/.kukuibot/src/certs/rootCA.pem \
  -d '{"to": "user@example.com", "subject": "Network Audit — Client — 2026-03-01", "html_path": "<OUTPUT_DIR>/report.html"}'
```
