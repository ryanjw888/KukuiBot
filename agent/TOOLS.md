...
_These are tools that can be added via plugins or custom scripts. They don't exist yet in this standalone app — they'll need to be built._

### Web Search 🔧 TODO
- Parallel search via Gemini (Google) + Grok (X)
- Returns combined results with source attribution
- Injection scanning on all results

### Image Generation 🔧 TODO
- Google Imagen 4.0 via Generative Language API
- Models: fast, standard, ultra
- Aspect ratios: 1:1, 16:9, 9:16, 4:3, 3:4

### Prompt Injection Guard 🔧 TODO
- DeBERTa-based injection detection (local model)
- Two-tier: fast regex pre-filter + model scoring
- HTTP service for scanning untrusted content

### Email Send 🔧 TODO
- SMTP with app-specific credentials
- Mandatory sanitization preflight before any send
- HTML email support for reports
