...
_OPENROUTER_BUILTIN_MODELS: dict[str, dict] = {
    ...
    "google/gemini-3.1-flash-lite-preview": {"label": "Gemini 3.1 Flash Lite", "max_tokens": 16384, "reasoning": "medium", "temperature": 0.4},
...
def _get_system_prompt(model_key: str = "", worker_identity: str = "", project_id: str = "") -> str:
    ...
    parts = [
        ...
        "FINISH WHAT YOU START. Do not stop after a few tool calls and summarize prematurely.",
        "When given a multi-step task, execute ALL steps using tools before responding.",
        "Do not say 'done' or provide a summary until you have actually completed every part of the task.",
        "If a task requires reading multiple files, making edits, and running tests — do all of it in one go.",
        "Only stop and ask for clarification if you are genuinely blocked, not because you want to check in.",
        "",
