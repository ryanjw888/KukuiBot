...
REPORT_DIR="$HOME/.macbot/reports/audit_${TIMESTAMP}"
mkdir -p "$REPORT_DIR"
...
MACBOT_URL="https://localhost:9000"  # MacBot API base URL
```
...
if [ -n "$MQTT_HOSTS" ]; then
  ...
  for H in $MQTT_HOSTS; do
    if which mosquitto_pub > /dev/null 2>&1; then
      ...
      if mosquitto_pub -h "$H" -p 1883 -t '__macbot_audit/probe' -m 'test' \
        --will-topic '__macbot_audit/gone' --will-payload '' \
...
Tell MacBot: *"Run a network audit"*
...
0 3 * * 0 /path/to/network_audit.sh >> ~/.macbot/logs/audit.log 2>&1
```

Or tell MacBot: *"Schedule a weekly network audit on Sundays at 3 AM"*
